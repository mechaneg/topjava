...
public class JdbcMealRepositoryTest {
    ...
    public void save() {
        {
            ...
            Meal savedMeal = repository.save(new Meal(newMeal), MealTestData.USER_ID);
        ...
        }
        {
            ...
            Meal updatedMeal = repository.save(new Meal(meal), MealTestData.USER_ID);
            MealTestData.assertMatchWithoutIds(meal, updatedMeal);
        }
        {
            // Trying update meal belonging to other user
            assertNull(repository.save(MealTestData.MEAL_1, MealTestData.ADMIN_ID));
        }
    }

    @Test
    public void delete() {
        assertTrue(repository.delete(MealTestData.MEAL_1_ID, MealTestData.USER_ID));
        assertFalse(repository.delete(MealTestData.MEAL_2_ID, MealTestData.ADMIN_ID));
        assertFalse(repository.delete(MealTestData.NON_EXISTING_MEAL_ID, MealTestData.USER_ID));
    }

    @Test
    public void get() {
        {
            Meal meal = repository.get(MealTestData.MEAL_1_ID, MealTestData.USER_ID);
            MealTestData.assertMatch(MealTestData.MEAL_1, meal);
        }
        {
            // Meal doesn't belong to user
            Meal meal = repository.get(MealTestData.MEAL_1_ID, MealTestData.ADMIN_ID);
            assertNull(meal);
        }
        {
            // Non-existing meal
            Meal meal = repository.get(MealTestData.NON_EXISTING_MEAL_ID, MealTestData.USER_ID);
            assertNull(meal);
        }
    }
    ...
    public void getAll() {
        {
            List<Meal> meals = repository.getAll(MealTestData.USER_ID);
            MealTestData.assertMatch(meals, MealTestData.MEAL_3, MealTestData.MEAL_2, MealTestData.MEAL_1);
        }
        {
            // Admin has no meals
            List<Meal> meals = repository.getAll(MealTestData.ADMIN_ID);
    ...
    public void getBetween() {
        {
            ...
            List<Meal> meals = repository.getBetween(beforeLunch, afterLunch, MealTestData.USER_ID);
        ...
        }
        {
            ...
            List<Meal> meals = repository.getBetween(leftTime, rightTime, MealTestData.USER_ID);
        ...
        }
        {
            ...
            List<Meal> meals = repository.getBetween(dayStart, dayEnd, MealTestData.ADMIN_ID);
            MealTestData.assertMatch(meals);
        }
    }
}